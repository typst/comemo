...
type Sink<'a, C> = &'a (dyn Fn(C, u128) + Send + Sync);
...
pub trait Track: Surfaces {
    ...
    #[inline]
    fn track_with<'a>(&'a self, sink: Sink<'a, Self::Call>) -> Tracked<'a, Self> {
        Tracked {
            value: self,
            sink: Some(sink),
            id: accelerate::id(),
...
#[inline]
pub fn to_parts_ref<T>(tracked: Tracked<'_, T>) -> (&T, Option<Sink<'_, T::Call>>)
where
    T: Track + ?Sized,
{
