...
impl<C: Call> Sink<C> for &'_ Mutex<Recording<C>> {
...
where
    In: Input + 'c,
    Out: Clone + 'static,
    F: FnOnce(In::WithLifetime<'c>) -> Out,
{
    ...
    if !enabled {
        ...
        let output = func(input.reborrow());
...
impl<C: Call, Out: 'static> CacheInner<C, Out> {
    ...
    fn lookup<In>(&self, key: u128, input: &In) -> Option<&CacheEntry<C, Out>>
    where
        In: Input<Call = C>,
    {
